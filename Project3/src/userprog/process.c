...
process_execute (const char *file_name) 
{
  ...
  if (tid == TID_ERROR){
	...
	struct list_elem *begin = list_begin(&childs);
	struct list_elem *end = list_end(&childs);
	struct thread *cur_child;

	while(begin != end){
		cur_child = list_entry(begin, struct thread, child);

		if(cur_child->exit_status == -1){
...
process_wait (tid_t child_tid UNUSED){
	...
	/* child process가 exeception 등으로 죽으면 return -1 */
	/* child process의 tid가 invalud해도 return -1 */
	/* process_wait을 호출한 process의 child가 아니어도 return -1 */
	/* tid에 대해서 process_wait()가 이미 호출됐어도 return -1 */
	...
	while(elem != end){
		cur_child = list_entry(elem, struct thread, child);

		if(cur_child->tid == child_tid){
			flag = true;
			break;
		}
		/* 다음 element가 없으면 return -1 */
		if(elem->next == NULL){
			return status;
		}
		elem = list_next(elem);
	...
	/* 같은 thread id를 갖는 child thread를 찾았으면 */
	if(flag){
